...
COPY system_files /
COPY --from=goodix-libfprint-builder /tmp/libfprint/build/libfprint/libfprint-2.so.2.0.0 /var/opt/libfprint-goodix/lib64/libfprint-2.so.2.0.0
...
RUN --mount=type=bind,from=ctx,source=/,target=/ctx \
  ...
  ln -sf libfprint-2.so.2.0.0 /var/opt/libfprint-goodix/lib64/libfprint-2.so.2 && \
  ln -sf libfprint-2.so.2 /var/opt/libfprint-goodix/lib64/libfprint-2.so && \
